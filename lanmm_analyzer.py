...
def plot_sim_results(sim_results,save_path=None,show=True):
    ...
    # Compute Welch PSD using 2-second segments
 # Compute Welch PSD using 2-second segments
    frequencies, psd = welch(sim_results['vP1'] + sim_results['vP2'], fs=fs, nperseg=fs*2)

    # Create figure and axes
    fig, ax = plt.subplots(figsize=(18, 5))

    # Plot the PSD line
    ax.plot(frequencies, psd, label='PSD (Welch)')

    # Find peaks in the PSD
    peaks, _ = find_peaks(psd, distance=10)

    # Filter peaks within the x-axis limits [0, 50] Hz
    valid_mask = (frequencies[peaks] >= 0) & (frequencies[peaks] <= 50)
    ...
    # Limit labeling to a maximum of 12 peaks:
    if len(valid_peaks) > 12:
